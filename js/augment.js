...
jQuery(document).ready(function () {
		...
		sw('.demo.hideThis').hide(); 
		// hide extra resources
		sw('.demo .openSlider').click(function(){
		  sw('.demo.hideThis').slideToggle();
		  sw('.demo.openRow').hide();
		  sw('.demo .openSlider').hide();
		});		

		sw('.webinar.hideThis').hide(); 
		// hide extra resources
		sw('.webinar .openSlider').click(function(){
		  sw('.webinar.hideThis').slideToggle();
		  sw('.webinar.openRow').hide();
		  sw('.webinar .openSlider').hide();
		});		

		sw('.whitepaper.hideThis').hide(); 
		// hide extra resources
		sw('.whitepaper .openSlider').click(function(){
		  sw('.whitepaper.hideThis').slideToggle();
		  sw('.whitepaper.openRow').hide();
		  sw('.whitepaper .openSlider').hide();
		});		

		sw('.casestudy.hideThis').hide(); 
		// hide extra resources
		sw('.casestudy .openSlider').click(function(){
		  sw('.casestudy.hideThis').slideToggle();
		  sw('.casestudy.openRow').hide();
		  sw('.casestudy .openSlider').hide();
		});		

		sw('.brochure.hideThis').hide(); 
		// hide extra resources
		sw('.brochure .openSlider').click(function(){
		  sw('.brochure.hideThis').slideToggle();
		  sw('.brochure.openRow').hide();
		  sw('.brochure .openSlider').hide();
		});		
      ...
      /*if (sw('#swrestricted').length > 0)
      {
        ...
        if(!validcountries.find(countrycode))
        {
			...
			// show direct link
			// <iframe  id="pardotForm" redirect_to="redirect_to=http%3A%2F%2Fwww.dynamics101.com%2Fresource-download%2F3&amp;offer_name=Sales+Order+Invoice+Process+in+AX+2012&amp;offer_category=Dynamics+AX"
			// get the redirect to (offer) URL
			var urlOffer = sw("#pardotForm").attr('redirect_to');						
			// detect if ? exists in url already
			
			var urlLink = sw("<a href='" + urlOffer + "'>Access Now</a>");
			sw('#pardotForm').after("<span class=error>" + restrictMsg + "</span>");
			sw('#pardotForm').after(urlLink);
